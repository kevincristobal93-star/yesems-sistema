...
const obtenerAdministradores = async () => {
  const resultado = await pool.query(
    'SELECT id_administrador, nombre, apellido, email, activo, created_at FROM administradores WHERE activo = true ORDER BY nombre ASC'
  );
...
const obtenerAdminPorId = async (id) => {
  const resultado = await pool.query(
    'SELECT id_administrador, nombre, apellido, email, activo, created_at FROM administradores WHERE id_administrador = $1',
    [id]
  );
  return resultado.rows[0];
};
...
const crearAdmin = async (datos) => {
  const { nombre, apellido, email, password_hash } = datos;
  const resultado = await pool.query(
...
const desactivarAdmin = async (id) => {
  const resultado = await pool.query(
    'UPDATE administradores SET activo = false WHERE id_administrador = $1 RETURNING id_administrador, nombre, apellido, email, activo',
    [id]
  );
  return resultado.rows[0];
};
...
const obtenerResumenPanel = async () => {
  const [cursos, alumnos, pagosPendientes, constanciasPendientes, ultimasInscripciones] = await Promise.all([
    ...
    pool.query(`SELECT i.id_inscripcion, i.fecha_inscripcion, i.estado, i.monto_total,
                       u.nombre AS usuario_nombre, u.apellido AS usuario_apellido,
                       c.nombre AS curso_nombre
                FROM inscripciones i
                JOIN usuarios u ON u.id_usuario = i.id_usuario
                JOIN cursos c ON c.id_curso = i.id_curso
                ORDER BY i.fecha_inscripcion DESC LIMIT 6`),
  ]);
  return {
    metricas: { cursos: cursos.rows[0].total, alumnos: alumnos.rows[0].total, pagos_pendientes: pagosPendientes.rows[0].total, constancias_pendientes: constanciasPendientes.rows[0].total },
    ultimas_inscripciones: ultimasInscripciones.rows,
  };
};

const obtenerPagosPendientes = async () => {
  const resultado = await pool.query(`
    SELECT p.id_pago, p.monto, p.metodo_pago, p.referencia, p.fecha_pago,
           i.id_inscripcion, c.nombre AS curso_nombre,
           u.nombre AS usuario_nombre, u.apellido AS usuario_apellido, u.folio
    FROM pagos p
...
const validarPago = async (idPago, estado) => {
  ...
  try {
    ...
    const pagoResult = await client.query(
      "UPDATE pagos SET estado = $1 WHERE id_pago = $2 AND estado = 'pendiente' RETURNING *",
      [estado, idPago]
    );
    ...
    if (estado === 'completado') {
      await client.query(
        "UPDATE inscripciones SET estado = 'confirmada' WHERE id_inscripcion = $1 AND estado = 'pendiente'",
        [pago.id_inscripcion]
      );
    }
...
const cancelarInscripcionAdmin = async (idInscripcion) => {
  ...
  try {
    ...
    const resultado = await client.query(
      "UPDATE inscripciones SET estado = 'cancelada' WHERE id_inscripcion = $1 AND estado <> 'cancelada' RETURNING *",
      [idInscripcion]
    );
    const inscripcion = resultado.rows[0];
