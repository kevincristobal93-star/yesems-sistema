...
async function loadCertificates() {
  ...
  try {
    ...
    document.querySelector('#certificate-guide-text').innerHTML = pending
      ? `<strong>${pending} solicitud${pending === 1 ? '' : 'es'} lista${pending === 1 ? '' : 's'}</strong> para revisión. Usa <strong>Autorizar y generar PDF</strong> para emitir la constancia.`
